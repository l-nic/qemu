...
// ********************************************
// Global lnic configuration interface.
...
void nic_t::set_own_port_id(uint64_t attempted_port_id) {
	if (!_enable) {
		return;
	}
	if (attempted_port_id == 0) {
...
// ***********************************************
// Support for the public lnic read interface.
...
uint64_t nic_t::num_messages_ready() {
	...
	if (num_read_messages != 0) {
		printf("Num read messages is %lu\n", num_read_messages);
	}
...
uint64_t nic_t::read_src_ip_lower() {
	if (!_enable) {
		return 0;
	}
...
uint64_t nic_t::read_src_ip_upper() {
	return 0; // Not yet supported
}

uint64_t nic_t::read_src_port() {
	if (!_enable) {
		return 0;
	}
...
// *************************************************
// Support for the public lnic write interface.
...
void nic_t::set_dst_ip_lower(uint64_t lower_ip_bits) {
	if (!_enable) {
		return;
	}
...
void nic_t::set_dst_port(uint64_t dst_port_id) {
	if (!_enable) {
		return;
	}
...
void nic_t::handle_lnic_datagram(uint8_t* datagram, size_t datagram_len, string src_ip_address) {
	...
	if (lnic_magic != LNIC_MAGIC) {
		printf("Lnic magic number not present\n");
...
void nic_t::listen_for_datagrams() {
	...
	while (true) {
		...
		printf("Received a raw datagram of length %lu with ip %s\n", bytes_received, fake_ip_address.c_str());
...
void nic_t::load_own_ip() {
	ifstream own_ip_file;
	own_ip_file.open(OWN_IP_FILENAME);
	string line;
	while (getline(own_ip_file, line)) {
		if (line.empty()) {
			continue;
		}
		_own_ip_address = line;
		break;
	}
	own_ip_file.close();
}
